  ...
  const fetchDeal = useCallback(async () => {
    ...
    if (res.ok) {
      ...
      setFields({
        ...
        demoDate: toDateInput(d.demoDate),
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          ...
          <div style={{ width: 340, flexShrink: 0, borderRight: '1px solid #e2e8f0', overflowY: 'auto', padding: '18px 18px 28px', background: '#fff' }}>
            ...
            <div style={{ marginBottom: 18 }}>
              {[['Enseigne', brand?.name], ['Magasin', store?.name], ['Ville', store?.city], ['Département', store?.department]].map(([l, v]) => (
            ...
            <div style={{ marginBottom: 18 }}>
              ...
              <div style={{ marginBottom: 9 }}>
                ...
                <input
                  type="date" style={inp} value={fields.demoDate ?? ''}
                  onChange={e => { setFields(f => ({ ...f, demoDate: e.target.value })); patchDeal({ demoDate: fromDateInput(e.target.value) }); }}
                />
              </div>
              <div style={{ marginBottom: 9 }}>
