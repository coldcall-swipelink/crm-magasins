...
function getBrandBorderColor(brandName?: string): string {
  ...
  for (let i = 0; i < brandName.length; i++) {
    h = Math.imul(31, h) + brandName.charCodeAt(i) | 0;
  }
  return '#' + Math.abs(h).toString(16).slice(0, 6).padEnd(6, '0');
}

function isRecentlyNew(createdAt: string): boolean {
  const diffDays = (new Date().getTime() - new Date(createdAt).getTime()) / (1000 * 60 * 60 * 24);
  return diffDays < 3;
}
...
export default function DealCard({ deal, isDragging, onDragStart, onDragEnd, onSelect }: Props) {
  ...
  return (
    ...
    >
      {brand && (
        <div style={{ fontSize: 9, fontWeight: 700, letterSpacing: '.5px', textTransform: 'uppercase', color: displayColor, marginBottom: 2 }}>
          {brand.name}
        </div>
      )}
      <div style={{ fontSize: 12, fontWeight: 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', color: '#0f172a' }}>
        {store?.name || 'Magasin'}
      </div>
      {store?.city && (
      ...
      {(deal as any).contactCalling && (
        <div style={{ fontSize: 10, color: '#4f46e5', marginTop: 3, fontWeight: 500 }}>
          📞 {(deal as any).contactCalling}
        </div>
      )}
