...
import { DEMO_MODE, demoUpdateAction, demoDeleteAction } from '@/lib/demo';
...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  try {
    ...
    if (DEMO_MODE) return NextResponse.json(demoUpdateAction(params.id, body));
...
export async function DELETE(_req: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    if (DEMO_MODE) { demoDeleteAction(params.id); return NextResponse.json({ success: true, demo: true }); }
    return NextResponse.json({ error: 'Erreur serveur' }, { status: 500 });
  }
}
